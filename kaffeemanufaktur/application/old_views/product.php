    ...
    function myFunction(per) {
        ...
        $.post('<?= base_url()?>products/select_type/' + id + '/' + idd,
            function(data, status, jqXHR) {
                ...
                var html = '<option value='' selected disabled hidden>Wählen Sie eine Option</option>';
                for (i = 0; i < obj.length; i = i + 1) {
                    if (i == 0) {
                         html += "<option value='" + obj[i].id + "' price='" + obj[i].price +"' selected>" + obj[i].size + "</option>";
                        $('#menge').append(html);
                        check_quantity();
                    } else {
                        html += "<option value='" + obj[i].id + "' price='" + obj[i].price + "'>" + obj[i].size + "</option>";
                        $('#menge').append(html);
                    }
                }
