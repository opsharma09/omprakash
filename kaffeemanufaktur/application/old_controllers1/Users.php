...
class Users extends CI_Controller
{
    ...
    public function user_profile($param1=''){
        ...
        if($param != ''){
            if($param1 == 'order'){
                $data['order']=$this->db->query("SELECT * FROM orders where user_id = $param")->result();
                foreach ($data['order'] as $key => $value) {
                    $count = $this->db->query("SELECT count(id) as pc FROM order_items where order_id = $value->id")->row();
                    $data['order'][$key]->item_count= $count->pc;
                }
