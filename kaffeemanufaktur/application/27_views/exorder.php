...
class Order extends CI_Controller
{
    ...
    public function save_order(){
        ...
        $body1 =  '<html>
                    ...
                    <p style="padding-left:10px">Ein Benutzer hat das Produkt bestellt. Folgende Bestellinformationen stehen zur Verfügung:</p>
                    
                    <p style="padding-left:10px">UserID: "'.$new_order->user_id.'"</p>
                    <p style="padding-left:10px">User Name: "'.$new_order->customer_name.'"</p>
                    <p style="padding-left:10px">Order ID: "'.$orderid.'"</p>
                    <p style="padding-left:10px">Transaction ID: "'.$new_order->txn_no.'"</p>
                    <p style="padding-left:10px">Total Order Price: "'.$new_order->total_amount.'"</p>';
        $body =  '<!DOCTYPE html>
                    <html lang="en">
                                ...
                                <tr>
                                    <td colspan="2">
                                        <div style="display: inline-table; width: 33%; vertical-align:middle; text-align: center;">
                                            <img src="images/email-logo.jpg" style="margin: 0 0 80px" alt="">
                                        </div>
                            ...
                            <tbody>
                                <tr>
                                    <td colspan="2">
                                        ...
                                        <h3 style="font-size: 15px; font-weight: 600; color: #1b1b1b; line-height: 26px; margin: 0 0 16px;">India</h3><a href="#" style=" display: inline-block;padding: 18px 29px;color: #fff;background: #940000;text-decoration: none;text-transform: uppercase;font-size: 18px;margin-bottom: 38px;">View or manage order</a>
                                        ...
                                        <p style="font-size: 15px; line-height: 1.7; color: #615f5f; margin: 0 0 1.5rem">Place on '.date('l', strtotime('+4 day', $added_on)).';, '.date('F', strtotime('+4 day', $added_on)).' '.date('d', strtotime('+4 day', $added_on)).', '.date('Y', strtotime('+4 day', $added_on)).'</p>
                                    </td>
                                </tr>';
        $body =  '<html><body>
                    <p>Hello, '.$session_data->first_name.' '.$session_data->last_name.'</p>
                    <p>Sie haben erfolgreich bei der Hannoversche Kaffeemanufaktur bestellt.</p>
                    <p style="padding-left:10px">Transaction ID: "'.$new_order->txn_no.'"</p>
                    <p style="padding-left:10px">Total Order Price: "'.$new_order->total_amount.'"</p>';                
        ...
        $body .= '<p><table border="1" style="padding:5px">
                        <tr>
                            <th>Image</th>
                            <th>Product name</th>
                            <th>Price</th>
        ...
        $body .='<tr>
                    <td><img src="'.base_url().$od['image1'].'" alt="" style="height:70px;width:40px"></td>
                    <td>'.$od['name'].'</td>
                    <td>'.$od['price'].'</td>
                    <td>'.$od['myQuan'].'</td>
                    <td>'.date('d m Y',strtotime($od['added_on'])).'</td>
                    <td>'.$event_type.'</td>
                   </tr>';

                }
        $body .='</table></p>'  ;      
        $body1 .='<p><table border="1" style="padding:5px">
                <tr>
                    <th>Image</th>
                    <th>Product name</th>
                    <th>Price</th>
       ...
       // var_dump($order_trips);
        ...
        $body1 .= '<p>Freundliche Grüße,</p>
                        <p>Ihre Hannoversche Kaffeemanufaktur</p>
                        </body>
                    </html>';
        $subject1 = "Neue Buchungsanfrage";            
        $body .= '</table></p><p>Freundliche Grüße,</p>
                <p>Ihre Hannoversche Kaffeemanufaktur</p>
                </body>
            </html>';
        $subject = "Vielen Dank für die Buchung einer Vergnügungsreise mit Hannoversche Kaffeemanufaktur";
        $this->send_smtp_mail($body1,$subject1,'rohit@enthuons.com', 'test001.enthuons@gmail.com');
        $this->send_smtp_mail($body,$subject,$session_data->email_id, 'test001.enthuons@gmail.com');
