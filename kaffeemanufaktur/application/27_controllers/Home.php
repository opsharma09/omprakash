...
class Home extends CI_Controller {
    ...
    public function gutschein()
    {
        ...
        if($this->input->get('attribute_art')=='Wertgutschein'){
            ...
            if($this->input->get('attribute_wert',TRUE)){
                ...
                $id = $product_info1['id'];
                $per = $per.',00';
                $availability = $this->db->query("select id,size,quantity,price,product_id,variation_id FROM availability where product_id = $id and size = '$per' order by price desc,size desc")->row_array();
                ...
                $availability = $this->db->query("select id,size,quantity,price,product_id,variation_id FROM availability where product_id = $product_info->id order by price desc,size desc")->result_array();
    ...
    public function create_subscription($token, $plan_id){
        $arr = array(
         [
            "plan_id" => $plan_id,
            "start_time" => "2021-03-11T00:00:00Z",
            "quantity" => "20",
            "shipping_amount" => [
                "currency_code" => "USD",
                "value" => "10.00"
    ...
    public function save_coupon_code(){
        $key = $_POST["key"];
        $email = $_POST["email"];
        $coupon_result = $this->db->query("SELECT * FROM `offers` WHERE `newsletter`= 'Y'")->row();
