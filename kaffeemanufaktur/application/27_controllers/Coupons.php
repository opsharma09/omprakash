...
class Coupons extends CI_Controller
{
    ...
    public function add()
    {
        if ($_POST){
                ...
                $coupon_valid_for = $_POST['coupon_valid_for'];
                ...
                $insert_array = array(
                    ...
                    'coupon_valid_for' => $coupon_valid_for,
    ...
    public function coupons_bulk_import(){
        ...
        if (!empty($_FILES['file']['name'])){
            $this->form_validation->set_rules('file', 'Uploaded file', 'callback_file_check');
            if($this->form_validation->run() == true){
            $extension = substr(strrchr($_FILES['file']['name'], '.'), 1);
            $actual_image_name = 'coupons' . time() . "." . $extension;
            if(move_uploaded_file($_FILES["file"]["tmp_name"], $target_path . $actual_image_name)){
                ...
                $file_path =  $target_path.$actual_image_name;
                    if ($this->csvimport->get_array($file_path)){
                        ...
                        foreach ($csv_array as $row) {
                            ...
                            $offer_value = $row['Offer Value'];
                            $description = $row['Description'];
                            $terms = $row['Terms'];
                            $min_cart_value = $row['Min Cart Value'];
                            $max_discount = $row['Max Discount'];
                            $coupon_valid_for = $row['Coupon Valid For'];
                            $allowed_user_times = $row['Allowed User Times'];
                            ...
                            if (!empty($check)){
                                ...
                                $this->home_m->insert_data('offer_category',array('offerID'=>$check->offerID,'category'=>$category,'subcategory'=>$subcategory,'created_at'=>date('Y-m-d H:i:s')));
                            }else{
                                $coupon_array['offerID'] = $offerID;
                    ...
                    $data['error_msg'] = 'Your file was not uploaded please try again here are your debug informations:'.print_r($_FILES);
                }
            }
        }
        $data['page_title'] = 'Coupons';
    ...
    if(!empty($posts))
    {
        foreach ($posts as $post)
        {
            ...
            $nestedData['Coupon Valid For'] = $post->coupon_valid_for;
            $nestedData['Allowed User Count'] = $post->allowed_user_times;
