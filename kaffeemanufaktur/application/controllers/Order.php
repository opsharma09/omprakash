...
class Order extends CI_Controller
{
    ...
    public function save_order(){
        ...
        $new_order =$this->db->query("SELECT * FROM orders where id = $orderid")->row();
        $monday_order = array(
            'first_name' => $user_bill_address->first_name,
            'last_name' => $user_bill_address->last_name,
            'email' => $user_bill_address->email,
            'phone' => $user_bill_address->phone,
            'address' => $user_bill_address->additional_address,
            'pincode' => $user_bill_address->pincode,
            'city' => $user_bill_address->place,
            'full_address' => $user_del_address->street.' '.$user_del_address->additional_address.' '.$user_del_address->pincode.' '.$user_del_address->place,
    ...
    public function check_coupons(){
        ...
        if(!empty($offers)){
            ...
            foreach ($offer_category  as $key => $value) {
                array_push($offer_cat, $value->category);
                array_push($offer_sub_cat, $value->subcategory);
            }
            $cat_arr = array_unique($cat_arr);
            $scat_arr =array_unique($scat_arr);
            // var_dump(array_diff($cat_arr, $offer_cat));
            // var_dump(array_diff($scat_arr, $offer_sub_cat));
            // $result ='';
            foreach ($offer_category  as $key => $value) {

                if($is_subscription == '' && $is_event == ''){
                    if((in_array($value->category, $cat_arr) && $value->subcategory == 0) || (in_array($value->category, $cat_arr) && in_array($value->subcategory, $scat_arr)) || $value->category == 0){
                        if(sizeof(array_diff($cat_arr, $offer_cat))<2 && sizeof(array_diff($scat_arr, $offer_sub_cat))<2){
    ...
    public function save_data_monday(){
        //eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjEwMTQ2NDUxMywidWlkIjoxNjQwMzc1MywiaWFkIjoiMjAyMS0wMy0wMlQwNTo1MjozMS4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6NzIyNjE5OCwicmduIjoidXNlMSJ9.qWbAyFbnDH9mIMLWAuFen9RQOoUk1tincBawx4lrb5c
        $token = "eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjEwMTQ2NDUxMywidWlkIjoxNjQwMzc1MywiaWFkIjoiMjAyMS0wMy0wMlQwNTo1MjozMS4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6NzIyNjE5OCwicmduIjoidXNlMSJ9.qWbAyFbnDH9mIMLWAuFen9RQOoUk1tincBawx4lrb5c";
        $apiUrl = 'https://api.monday.com/v2';
        $headers = ['Content-Type: application/json', 'Authorization: ' . $token];
        $todaydate=date("Y-m-d");
        $query = 'mutation ($myItemName: String!, $columnVals: JSON!) { create_item (board_id:773499757, item_name:$myItemName, column_values:$columnVals) { id } }';
        $vars = ['myItemName' => 'op new item', 
          'columnVals' => json_encode([
            'status_1' => ['label' => 'Bitte auswählen'], 
            'status' => ['label' => ''],
            'date4' => ['date' => $todaydate],//today date 
            'text'=>"Test First Name",
            'text9'=>"Test Last Name",
            'email' =>[
              "email"=> "itsmytestemail@mailserver.com",
              "text"=> "my test email"
            ],
            'phone6' =>[
           "phone"=> "11231234567"
            ],
            'text_17' => "Test Address ",
            'text_10' => "PLZ",
            'text_14' => "Test City ",
                'long_text' => "Test Shipping address",
                'text_16' => "Test payment method",
                'numbers5' => "444",
                'numbers4' => "4464",
        ])];
