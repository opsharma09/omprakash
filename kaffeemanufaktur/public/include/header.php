...
<div id="top_menu_wrapper">
    <div id="top_menu_right">
        ...
        <?php if($this->session->userdata('user')){?><a href="<?=base_url('mein-account')?>" id="header_account">Hallo, <span id="username_login"><?php $arr = explode("@", $this->session->userdata('user')->email_id, 2);echo $arr[0];  ?>!</span></a><?php }else{?><a href="<?=base_url('login/logout')?>">Einloggen</a> / <a href="<?=base_url('register')?>">Registrieren</a></li><?php } ?>
    </div>
                ...
                <ul class="header">
                                ...
                                <div class="col-md-6">
                                    <ul>
                                        ...
                                        <?php foreach ($subcategory as $sub_key => $sub_cat) {
                                            ...
                                            ?>
                                            <li><a href="<?= base_url('produkt-kategorie/kaffee-espresso/'.com_slugify($cat->name)).'/'.com_slugify($sub_cat['e_name']) ?>"><?=$sub_cat['name']?></a></li>
                                        <?php } ?>
                                    </ul>
                                </div>
                                <div class="col-6">
                                    <ul>    
                                        ...
                                        <?php } ?>
                                        <li class="bold"><a href="<?= base_url('produkt-kategorie/kaffee-espresso/'.com_slugify($cat->name)) ?>">Noch mehr Auswahl</a></li>
                    ...
                    <?php }elseif($key == 1){?>
                        ...
                        <ul class="sub-menu menu_dropdown">
                            ...
                            <li><a href="<?=base_url('kaffee-finder')?>">Meinen Lieblingskaffee finden</a></li>
                            <li><a href="<?= base_url('produkt-kategorie/kaffee-espresso/'.com_slugify($cat->name)) ?>">Unsere Espressi</a></li>
                            <?php foreach ($subcategory as $sub_key => $sub_cat) {
                                $products=$this->Common->select_data('products','*',array('category'=>$cat->id,'subcategory' => $sub_cat['id']));
                                ?>
                            <li><a href="<?= base_url('produkt-kategorie/kaffee-espresso/'.com_slugify($cat->name)).'/'.com_slugify($sub_cat['e_name']) ?>"><?=$sub_cat['name']?></a>
                                
                            </li>
                        <?php } ?>
                            <li><a href="<?= base_url('produkt-kategorie/kaffee-espresso/'.com_slugify($cat->name)) ?>">Noch mehr Auswahl</a></li>
                    ...
                    <li><a href="<?=base_url('produkt-kategorie/geschenke')?>">Geschenke</a>
                        <ul class="sub-menu menu_dropdown">
                            ...
                            <li class="margin-up"><a href="<?=base_url()?>produkt/gutschein/?attribute_art=Wertgutschein">Wertgutscheine</a></li>
                            <li><a href="<?=base_url()?>produkt/gutschein/?attribute_art=Wertgutschein&attribute_wert=10,00€">10,00 €</a></li>
                            <li><a href="<?=base_url()?>produkt/gutschein/?attribute_art=Wertgutschein&attribute_wert=15,00€">15,00 €</a></li>
                            <li><a href="<?=base_url()?>produkt/gutschein/?attribute_art=Wertgutschein&attribute_wert=20,00€">20,00 €</a></li>
                            <li><a href="<?=base_url()?>produkt/gutschein/?attribute_art=Wertgutschein&attribute_wert=50,00€">50,00 €</a></li>
                            <li><a href="<?=base_url()?>produkt/gutschein/?attribute_art=Wertgutschein&attribute_wert=100,00€">100,00 €</a></li>
                        </ul>
                    </li>
                    ...
                    <?php if($this->session->userdata('user')){$event_cart_count=0;$cart_c_count=0;
                        $cartt_count =$this->Common->select_data('add_to_cart','sum(qty) as cart',array('user_id'=>$this->session->userdata('user')->user_id));
                        $eventt_cart =$this->Common->select_data('event_booking','count(id) as cart',array('user_id'=>$this->session->userdata('user')->user_id,'is_booked'=>0));
                        $event_cart_count = $eventt_cart[0]['cart'];
                        $cart_c_count = $cartt_count[0]['cart'];
                    ?><li><a href="<?= base_url() ?>home/cart" style="color:white"><?php if($cart_c_count||$event_cart_count){?><i class="fas fa-shopping-cart"><span style="font-size:15px;margin-bottom-3px"><?=$cart_c_count+$event_cart_count?> </span></i></a><?php }else{?><a href="<?= base_url() ?>home/cart" style="color:white"><i class="fas fa-shopping-cart"></i></a></li>
                    <?php } } else{ ?>
