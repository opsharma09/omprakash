...
<div style="background: black;height:30px;">
    <ul class="navigation clearfix">
        ...
        <?php if($this->session->userdata('user')){?><li style="float: right;margin-right: 10px;padding:5px"><a href="<?=base_url('mein-account')?>">Hallo, <?php $arr = explode("@", $this->session->userdata('user')->email_id, 2);echo $arr[0];  ?>!</a></li><?php }else{?><li style="float: right;margin-right: 10px;padding:5px"><a href="<?=base_url('login/logout')?>">Einloggen</a> / <a href="<?=base_url('register')?>">Registrieren</a></li><?php } ?>
        <li style="float: right;margin-right: 10px;padding:5px"><a href="<?=base_url('kontakt')?>">Kontakt</a></li>
        ...
        <div class="row no-gutters">
            <div class="col-12 desktop">
                    ...
                    <li lass="menu">
                        ...
                        <?php } ?>
                            ...
                            foreach ($subcategory as $sub_key => $sub_cat) {
                                ?>
                            <li><a href="<?= base_url('produkt-kategorie/kaffee-espresso/'.com_slugify($cat->name)).'/sortenreine_kaffees/'.com_slugify($sub_cat['e_name']) ?>"><?=$sub_cat['name']?></a>
                                <ul class="sub-menu sub_menu_dropdown">
                                    <?php if(!empty($products)){ foreach($products as $prod){?>
                                        <li><a href="<?=base_url('produkt/'.com_slugify($prod['e_name']))?>"><?=$prod['name']?></a></li>
                                    <?php } } ?>
                ...
                <?php } }?>
                    ...
                    ?>
                    <li><a href="<?= base_url() ?>home/cart">Cart <?php if($cart_count[0]['cart']){?><span style="font-size:15px;margin-bottom-3px">( <?=$cart_count[0]['cart']?> )</span><?php }?></a></li>
                    <?php } else{ ?>
